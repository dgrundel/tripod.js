...
```
var myAwesomeInstance = new Triangular([[[(object) initialAttributes], (string) namespace], (boolean) persist]);
```
...
```
var myAwesomeInstance = new Triangular();
```
...
var firstCar = new Triangular(attributes, namespace, persistToLocalStorage);
```
...
The namespace tells Triangular which nodes in the DOM belong to this instance. We'll talk about that a little more in minute.

The last argument is a boolean that tells Triangular whether or not to store data in Local Storage as well.
...
Triangular needs you to tell it which DOM nodes (inputs, checkboxes, paragraphs, etc) to pay attention to.
To do that, you need to add a little data attribute to elements you want to sync with your Triangular instance.
...
If you passed _true_ as the third argument in the constructor, your Triangular instance will be backed by Local Storage. 
...
Javascript:
```
var address = new Triangular({city: '', state: ''}, 'address');
address.loadAll();
...
var address = new Triangular(addressData, 'address');
